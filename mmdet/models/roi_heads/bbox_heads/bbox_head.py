...
class BBoxHead(BaseModule):
    ...
    def loss(self,
        ...
        decoded_pred = self.bbox_coder.decode(rois[:, 1:], bbox_pred)
        decoded_tar = self.bbox_coder.decode(rois[:, 1:], bbox_targets)
        IoU = bbox_overlaps(decoded_pred,decoded_tar,is_aligned=True).reshape(512,-1)#shape (1024,) seperate batchsize bbox
        pos_neg_weight = bbox_weights[:,0].reshape(512,-1)
        mIoU = torch.sum(IoU*pos_neg_weight,dim=0)/torch.sum(pos_neg_weight,dim=0)
        re_weight = 2/(1+torch.exp(-2*(mIoU-IoU)))

        bbox_weights = bbox_weights*re_weight.reshape(-1,1)
        # print(bbox_weights)
        '''
    ...
    def get_bboxes(self,
        ...
        if bbox_pred is not None:
            ...
            save_list = []
            save_list.append(bboxes)
            # np.save('box_head_before_nms.npy', save_list, allow_pickle=True)
            # print(bboxes)
        else:
            bboxes = rois[..., 1:].clone()
