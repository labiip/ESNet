...
class PABBoxHead(BaseModule):
    ...
    def __init__(self,
                 ...
                 loss_as = dict(type='MSELoss',
                                   reduction='mean', loss_weight=0.5),####################
                 init_cfg=None):
        super(PABBoxHead, self).__init__(init_cfg)
        ...
        ####here
        #self.iou = build_iou_calculator(dict(type='BboxOverlaps2D',))
                                             

        self.bbox_coder = build_bbox_coder(bbox_coder)
        self.loss_cls = build_loss(loss_cls)
        self.loss_bbox = build_loss(loss_bbox)
        self.loss_as = build_loss(loss_as) ################
        ####here
        #self.iou_loss = build_loss(dict(type='SmoothL1Loss', beta=1.0, loss_weight=1.0))
        ...
        self.share_fc_1024 = build_linear_layer(
                self.cls_predictor_cfg,
                in_features=in_channels,
                out_features=1024)
        ######as-panet model ######
        self.count_num_1 = nn.Linear(25600, 256)
        self.count_num_2 = nn.Linear(256,1 )
        #self.count_num = nn.Linear(217600, 1)
    ...
    @auto_fp16()
    def forward(self, x,muti_features):
        ###as branch###
        x1 = self.trans1(muti_features[0]).view(muti_features[0].size(0),-1)
        x2 = self.trans2(muti_features[1]).view(muti_features[1].size(0), -1)
        x3 = self.trans3(muti_features[2]).view(muti_features[2].size(0), -1)
        x4 = self.trans4(muti_features[3]).view(muti_features[3].size(0), -1)
        fc_fusion_feature = x1+x2+x3+x4
        num_out = self.count_num_2(self.relu(self.count_num_1(fc_fusion_feature)))

        
        ###input x is [1,2,3,4],conv and max fusion operation
        for i in range(len(x)):
            x[i] = self.mask_conv1[i](x[i])
        #max fusion operation
        ...
        if self.with_avg_pool:
            x = self.avg_pool(x)
        x = x.view(x.size(0), -1) #shape [batch_size,n]
        x = self.share_fc_1024(x)
        cls_score = self.fc_cls(x) if self.with_cls else None
        bbox_pred = self.fc_reg(x) if self.with_reg else None
        print(num_out)
        
        return cls_score, bbox_pred,num_out
    ...
    def get_num_target(self,gt_bboxes):
        list = []
        for i in gt_bboxes:
            list.append(len(i))
        num_target = torch.FloatTensor(list).view(len(gt_bboxes),-1)
        return num_target
    ...
    def get_targets(self,
        ...
        ####modify here ####
        #iou_weights = bbox_weights[:,0]
        return labels, label_weights, bbox_targets, bbox_weights#,iou_weights

    @force_fp32(apply_to=('cls_score', 'bbox_pred','num_out'))
    def loss(self,
             cls_score,
             bbox_pred,
             num_out,########################
        ...
        if num_out is not None:
            num_target = torch.FloatTensor(num_target).cuda()
            #num_out = num_out.floor().float()
            num_thr = torch.FloatTensor([[38,38]]).cuda() # put on 0 number GPU,
            #vs = torch.gt(num_out,num_thr)+0
            # if vs.sum()>=2:
            #    num_out = (num_out.floor()+1).float()
            #num_out = torch.ceil(num_out)
            #if (num_out > 38).any():
            #  num_out = torch.ceil(num_out)
            #else:
            #  pass
            ...
            try:
                loss_as = self.loss_as(num_out,num_target)
                if isinstance(loss_as, dict):
                    losses.update(loss_as)
                else:
                    losses['loss_as'] = loss_as
            except:
                  pass
        ...
        if bbox_pred is not None:
            ...
            if pos_inds.any():
                ...
                #losses['loss_iou'] = self.iou_loss(iou_pred,IOU_target,iou_weights,avg_factor=bbox_targets.size(0))##################
            else:
                losses['loss_bbox'] = bbox_pred[pos_inds].sum()
                #iou_loss = iou_pred*iou_weights ###################
                #losses['iou_loss']  = iou_loss.sum() ################
        return losses
