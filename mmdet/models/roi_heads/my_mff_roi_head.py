...
class MyMFFRoIHead(BaseRoIHead, BBoxTestMixin, MaskTestMixin):
    ...
    def forward_train(self,
        ...
        if self.with_mask:
            mask_results = self._mask_forward_train(x, sampling_results,
                                                    ...
                                                    gt_masks, img_metas)
            losses.update(mask_results['loss_mas'])
    ...
    def _mask_forward_train(self, x, sampling_results, bbox_feats, gt_masks,
        ...
        #    mask_targets[0].unsqueeze(1),
        #    x[0].shape[-2:], mode='bilinear', align_corners=False).squeeze(1)
        #semantic_target = (semantic_target >= 0.5).float()
        
        #mask_targets[0] = semantic_target
        
        pos_labels = torch.cat([res.pos_gt_labels for res in sampling_results])
        
        loss_mask = self.mask_head.mutil_loss(mask_results['mask_pred'],
                                        mask_targets, pos_labels)

        mask_results.update(loss_mas=loss_mask, mask_targets=mask_targets)
        return mask_results
    ...
    def _mask_forward(self, x, rois=None, pos_inds=None, bbox_feats=None):
        ...
        #print('the shape is',x[0].shape)
        # if self.with_shared_head:
        #     mask_feats = self.shared_head(mask_feats)

        mask_pred = self.mask_head(mask_feats,x[0],rois)
    ...
    def simple_test_mask(self,
        ...
        mask_results = self._mask_forward(x, mask_rois)
        # print(mask_results['mask_pred'].equal(mask_results['mask_edge']))
        mask_pred = mask_results['test_item']
        # mask_pred = mask_results['mask_edge']
