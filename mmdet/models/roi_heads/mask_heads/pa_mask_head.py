...
class PAFCNMaskHead(BaseModule):
    ...
    def __init__(self,
        ...
        super(PAFCNMaskHead, self).__init__(init_cfg)
        ...
        self.conv4 = nn.Sequential(
                ...
                nn.BatchNorm2d(256, eps=1e-05,momentum=0.1, affine=True),
                nn.ReLU(inplace=True))
        self.conv4_fc = nn.Sequential(
                nn.Conv2d(in_channels, in_channels, 3, 1, padding=1, bias=False),
                nn.BatchNorm2d(256, eps=1e-05,momentum=0.1, affine=True),
                nn.ReLU(inplace=True))
        self.conv5_fc = nn.Sequential(
                nn.Conv2d(in_channels, in_channels//2, 3, 1, padding=1, bias=False),
                nn.BatchNorm2d(128, eps=1e-05,momentum=0.1, affine=True),
                nn.ReLU(inplace=True))
        self.mask_fc = nn.Sequential(
                nn.Linear(int(in_channels / 2) * (roi_feat_size) ** 2,  28** 2, bias=True),
                nn.ReLU(inplace=True))
    def init_weights(self):
        super(PAFCNMaskHead, self).init_weights()
        ...
        for m in [self.conv4,self.conv4_fc,self.conv5_fc]:
            for n in m:
        ...
        for m in self.mask_fc:
            for n in m:
                if isinstance(n,nn.Linear):
                    ...
                    torch.nn.init.constant_(n.bias, 0)
    ...
    def forward(self, x):
        ...
        for conv in self.convs: #has modified ,there is 3 convs
            x = conv(x)
        mask_cov = x
        mask_fc = x 
        batch_size = x.size(0)
        #here seperate 
        mask_fc_out = self.conv4_fc(mask_fc)
        mask_fc_out = self.conv5_fc(mask_fc_out).view(batch_size,-1) #shape[B,128×14×14]
        mask_fc_out = self.mask_fc(mask_fc_out).view(batch_size,1,28,28)
        
        if self.upsample is not None:
            mask_cov = self.upsample(mask_cov)
            if self.upsample_method == 'deconv':
                mask_cov = self.relu(mask_cov)
        mask_conv_out = self.conv_logits(mask_cov)
        
        
        #here convs and fc layer element-wise add 
        mask_pred = mask_conv_out+mask_fc_out
        return mask_pred
