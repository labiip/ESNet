...
class myhead(BaseModule):
    ...
    def __init__(self,
        ...
        super(myhead, self).__init__(init_cfg)
        ...
        self.gcn = gcn(256)
        self.gcn_forconv=nn.Conv2d(256,256,kernel_size=3,padding=1)
        ...
        self.mask_out = nn.Conv2d(256,1,kernel_size=1,padding=0,stride=1)
        self.edge_out = nn.Conv2d(256,1,kernel_size=1,padding=0,stride=1)
        ...
        self.extractor = build_roi_extractor(self.extractor_dict)
        self.segmantic_out = nn.Conv2d(256,1,kernel_size=1,padding=0,stride=1)
        
        self.jiangwei = nn.Conv2d(512,256,kernel_size=1,padding=0,stride=1)
        self.edge_14 = Conv2d(256,256,kernel_size=3,padding=1,stride=1)
        self.edge_28 = Conv2d(256,256,kernel_size=3,padding=1,stride=1)
        

    def init_weights(self):
        super(myhead, self).init_weights()
        nn.init.constant_(m.bias, 0)
  
        for m in [self.upsample, self.conv_logits,self.upsample_ea]:
            if m is None:
                continue
    ...
    def forward(self, x):
        ...
        mask_feature = self.relu(self.upsample(x))
        #edge_feature = self.relu(self.upsample_ea(x))
        
        mask = self.mask_out(mask_feature)
        edge = self.edge_out(mask_feature)
        return mask,edge
    ...
    def get_targets(self, sampling_results, gt_masks, rcnn_train_cfg):
        ...
        mask_targets_28 = mask_target(pos_proposals, pos_assigned_gt_inds,
                                   gt_masks, rcnn_train_cfg)
                                   
        #dic = ConfigDict(mask_size=14)
        #mask_targets_14 = mask_target(pos_proposals, pos_assigned_gt_inds,
                                   #gt_masks, dic)
                                   
        # edge_targets_14 = self.get_edge(mask_targets_14)
 
        #edge_targets_28,edge_weight = self.get_edge(mask_targets_28)
        ...
        #max_h = max([target.shape[-2] for target in segmantic_list])
        #max_w = max([target.shape[-1] for target in segmantic_list])
        
        #segmantic_target = torch.zeros(
        #    (len(segmantic_list), max_h, max_w),
        #    dtype=segmantic_list[0].dtype, device=segmantic_list[0].device)
            
        #for idx, target in enumerate(segmantic_list):
        #    segmantic_target[idx, :target.shape[-2], :target.shape[-1]] = target
            
        #print(segmantic_target.shape)
        
        return mask_targets_28#,edge_targets_28,edge_weight
    ...
    def loss(self, mask_pred, mask_targets, labels):
        ...
        """
        #print(mask_pred.shape)  mask_pred shape [n,1,28,28]
        #print(mask_targets.shape) mask_edge shape [n,28,28]
        loss = dict()
        """
    ...
    def loss_e(self, edge_pred, edge_targets, edge_targets_w):
        ...
        #loss_edge_dice = dice_loss_func(torch.sigmoid(edge_pred),edge_targets)
    ...
    def get_edge(self, gt_masks): 
        ...
        gt_masks = gt_masks[:, None, :, :] #Ôö¼ÓÎ¬¶È
