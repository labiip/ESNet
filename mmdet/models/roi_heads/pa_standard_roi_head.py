...
from .pa_base_roi_head import PABaseRoIHead
...
class PANetRoIHead(PABaseRoIHead, BBoxTestMixin, MaskTestMixin):
    ...
    def init_mask_head(self, mask_roi_extractor_lev1,mask_roi_extractor_lev2,mask_roi_extractor_lev3,mask_roi_extractor_lev4, mask_head):
        ...
        if mask_roi_extractor_lev1 is not None:
    ...
    def forward_train(self,
        ...
        if self.with_as:
            ...
            print('output is ',count_num)
            print('target is ',count_num_target)
        ...
        if self.with_bbox:
            ...
            """
            if self.with_as:
                bbox_results['loss_bbox'].update(loss_as=loss_as)
    ...
    def _bbox_forward(self, x, rois):
        ...
        for i in range(len(bbox_feats)):
            for j in range(i+1,len(bbox_feats)):
                print(bbox_feats[i].equal(bbox_feats[j]))
        """
        # next step is feats fusion by max operation
        if self.with_shared_head:
            bbox_feats = self.shared_head(bbox_feats)
        #cls_score, bbox_pred ,iou_pred= self.bbox_head(bbox_feats)
        cls_score, bbox_pred, num_out = self.bbox_head(bbox_feats,x)
        
        bbox_results = dict(
            cls_score=cls_score, bbox_pred=bbox_pred, bbox_feats=bbox_feats,num_out=num_out)
        return bbox_results
    ...
    def _bbox_forward_train(self, x, sampling_results, gt_bboxes, gt_labels,
        ...
        num_targets = self.bbox_head.get_num_target(gt_bboxes)
        ###########
        ...
        '''
        loss_bbox = self.bbox_head.loss(bbox_results['cls_score'],
                                        bbox_results['bbox_pred'],bbox_results['num_out'],rois,
                                        *bbox_targets,num_targets)
